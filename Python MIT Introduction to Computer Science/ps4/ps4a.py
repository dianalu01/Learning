...
def displayHand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]):
            result=result+letter+" "       # print all on the same line
    print result
...
def isValidWord(word, hand, wordList):
    ...
    try:
        ...
        if word in wordList:            
            for letter in word:  
                if copyHand.get(letter, -1)<=0:
                    return False
                else:
                    copyHand[letter]=copyHand[letter]-1 
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        # Display the hand
        print("Current Hand:"+returnHand(hand))
        # Ask user for input
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                score += wordScore
                print('"{}" earned {} points. Total: {} points'.format(word,wordScore, score))
                # Update the hand
...
def playGame(wordList):
    ...
    userInput = 'n'
    while True:
        userInput = str(input('Enter n to deal a new hand, r to replay the last hand, or e to end game: r '))
        if userInput=='r':
            if hand:
